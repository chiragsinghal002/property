...
// session_start();
		...
		$getrequirementresi=$common->getResiRequirementByDealerId($_SESSION['dealer_id']);
   ...
   // var_dump($getrequirementresi);
    // var_dump($comm_properties);
		...
		<?php if(!empty($getrequirementresi)){?>
			...
			<?php foreach($getrequirementresi[0] as $data){?>
...
// session_start();
	$type='Expired';
	$getexprequirementresi=$common->getResiExpRequirementByDealerId($_SESSION['dealer_id']);
	$getexprequirementcomm=$common->getExpCommRequirementByDealerId($_SESSION['dealer_id']);
      // var_dump($getexprequirementresi);
	if(!empty($getexprequirementresi)){
		$exp_prop=count($getexprequirementresi);
	}else{
		$exp_prop=0;
	}
	...
	<!--  Land For Sale: -->
	<?php if(!empty($getexprequirementresi)){?>
		<?php foreach($getexprequirementresi as $data){?>
			<?php 
			$today=date('Y-m-d H:i:s');
